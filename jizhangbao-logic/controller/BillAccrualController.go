...
func GetMonthBillAccrualPage(params map[string]string) mgresult.Result {
	if !utils.Exists(params, "bookId") {
		return *mgresult.Error(-1, "缺少账本编号参数")
	}
	inOut := 0
	...
	return service.GetMonthBillAccrualPage(params["bookId"], params["month"], params["billTypeId"], params["customerId"], inOut, excuteDate)
}
...
func AddBillRecord(params map[string]string) mgresult.Result {
	...
	amount, _ := strconv.ParseFloat(params["amount"], 64)
	return service.AddBillRecord(params["bookId"], params["billId"], params["billAccountId"], params["remark"], params["photos"], amount, sync)
}
