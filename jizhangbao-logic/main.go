...
const mgconfig_file = "jzb-logic.yml"

//@title	记账宝业务逻辑服务模块API
//@version 	1.0.0(jzb-logic)
//@description	记账宝业务逻辑服务模块
func main() {
	//初始化配置，自动连接数据库和Nacos服务注册
	path, _ := filepath.Abs(filepath.Dir(os.Args[0]))
	mgconfig.InitConfig(path + "/" + mgconfig_file)
	...
	//common.PrintLogo()
	fmt.Println("|-----------------------------------|")
	fmt.Println("|       记账宝Jzb-Logic 0.0.1        |")
	fmt.Println("|-----------------------------------|")
	fmt.Println("|  Go Http Server Start Successful  |")
	fmt.Println("|    Port:" + mgconfig.GetConfigString("go.application.port") + "     Pid:" + fmt.Sprintf("%d", os.Getpid()) + "        |")
	fmt.Println("|-----------------------------------|")
	fmt.Println("")
